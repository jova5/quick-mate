...
const EditPostScreen = ({post}:{post: PostInterface}) => {
  ...
  const [price, setPrice] = useState<string>(post.price.toString());
  const [contactNumber, setContactNumber] = useState<string>(post.contactPhoneNumber);
  ...
  useEffect(() => {
    ...
    dispatch(setSelectedCityName(user?.cityName!));
